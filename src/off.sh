        ...
        for os in "${os_list[@]}";do
            ...
            for is in "${is_list[@]}";do
                ...
                for ((s=0;s<$N;s+=1));do
                    for ((i=$s;i<$mx;i+=N));do
                        echo ${i}${name}
                        python3 -u pa_meta.py --outdir $name --env $env --gpu $(($i%2)) --seed $i --v_learn_epochs $v_learn_epochs --innerepochs $innerepochs --innerstepsize $innerstepsize --outerstepsize outerstepsize;
                    done&
                done;
            done;
        done;
